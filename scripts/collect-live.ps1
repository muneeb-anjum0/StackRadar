...
$mode = if ($env:PIPELINE_MODE) { $env:PIPELINE_MODE } else { "direct" }

python pipelines/collectors/live_collect.py --source all --limit $limit --query $query --country $country --mode $mode @args
