...
export function UsagePanel({ usage }: { usage?: AiUsage }) {
  return (
    <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50/70 p-5">
      ...
      <p className="mt-4 text-xs leading-5 text-slate-500">Cooldown: {usage?.cooldown_seconds ?? 20}s. Latest OpenRouter: {usage?.latest_openrouter_report_at ? new Date(usage.latest_openrouter_report_at).toLocaleString() : "none"}</p>
    </div>
  );
}
